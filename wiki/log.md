...
**更新页面**：[[aten算子调用链定位方法论]] 第二节新增"双层模型""注册表项""完整分析链路"三小节；修正两处易误读表述；reshape 在 2.12 yaml:5141（Composite）、view yaml:8422（有 dispatch 段）、_reshape_alias yaml:5157（有 dispatch 段）。
...
## [2026-07-31] fix | 完整链路图重写：信息分层 + pip 自带头文件实证

用户反馈：链路图布局和描述都不清晰（运行时链路、生成器归属、验证手段三类信息挤在 ASCII 树里，注释列对不齐）。

**重写**：[[aten算子调用链定位方法论]]"完整分析链路"拆为——①纯运行时链路图（每行一个环节+一句话角色，无验证信息）；②分层明细表（产物文件/生成器/验证分"无需 build"与"需 build"两列）。

**重要新发现（本机 pip torch 2.12 实测）**：pip 版自带 `torch/include/ATen/ops/*.h` 生成头文件，无需 build 即可实证包装层——`flatten.h` 里 `at::flatten()` 的整个 body 就是一行 `return at::_ops::flatten_using_ints::call(...)`；`flatten_compositeimplicitautograd_dispatch.h` 旁证注册 key。且每个生成文件头注释自带 `@generated by {生成器} from {模板}`——"文件自己报出生"成为通用判别动作。

## [2026-07-31] update | ④ 断点详解：控制流转数据流，注册项的正确搜法

用户实操走到 ③→④：在 `build/aten/src/ATen/Operators_3.cpp` 找到了 `flatten_using_ints::call` 的 body（`create_flatten_using_ints_typed_handle()` + `op.call(...)`），但不知道下一步怎么走——"找不到"。

**核心解答**：③→④ 是整个链路唯一的"控制流转数据流"断点。`op.call` 往里（Dispatcher::call，Dispatcher.h:179）是 2000+ 算子共用的通用路由机器，无任何算子专属代码，顺调用点读永远读不到 flatten。绑定关系是加载期（import torch 时，main 之前）由 `TORCH_LIBRARY_IMPL` 静态初始化通过 `m.impl(...)` 写进全局表的数据——要找"注册"不是找"调用"。
...
## [2026-07-31] add | 补齐第三个粘合页：LLM 与 MoE 模型基础

上轮重组留下两个待定项，用户指示继续——先把"每个大类都有粘合页"的结构补全（推荐阅读顺序的微调仍待用户反馈）。

**新建**：[[LLM 与 MoE 模型基础]]（hub）——主线"模型结构是什么、如何决定下层系统/算子形态"；阅读顺序：LLM 前向计算（地基：计算/访存判据）→ MoE（核心结构）→ Shared Experts / Latent MoE（扩展）→ DeepSeek（典型代表）；逻辑关系图 + 与 vLLM/PyTorch/FlagGems 大类的接口（Grouped GEMM / moe_align / EP / Triton / Dispatcher）。

**同步**：[[index]] 该大类标注粘合页 + 补一句主线描述；[[vLLM 推理框架]] 阅读顺序第 1-2 步标注"属模型基础大类"（避免两 hub 争归属）；[[overview]] 29 页、大类导航补齐并清掉一行旧重复条目。全部 wikilink 脚本验证零断链。

**结构定稿**：4 个大类 = 3 个 hub（PyTorch 源码机制 / vLLM 推理框架 / LLM 与 MoE 模型基础）+ 1 个单页大类（FlagGems，内容增多后再升级 hub）。

## [2026-07-31] fix+rule | 大类命名原则纠正 + 大类自动演化立法

用户两点批评：①"PyTorch 源码机制"作为大类名太片面——想了解的可能是 PyTorch 的实现/算法而非机制；②大类以后会增加但用户不会显式告知，需要 LLM 在沉淀时发现新大类就自动创建，且这条规则要写入操作文件。

**命名纠正（原则：大类名 = 领域名，不是当前内容的角度名；角度降级为 hub 内的"子线"）**：
- hub 改名 [[PyTorch 源码机制]] → [[PyTorch]]：范围扩为"PyTorch 的一切（机制/实现/算法/API/生态）"，现有内容标注为"源码机制子线（当前唯一子线）"，新角度内容直接在 hub 内加子线
- hub 改名 [[LLM 与 MoE 模型基础]] → [[LLM 与 MoE 模型]]：同理，"模型基础与结构"降级为子线
- [[vLLM 推理框架]] 保留：领域名 vLLM 已被实体页 [[vLLM]] 占用，按撞名规则用大领域短语
- 引用同步：[[index]]、[[overview]]（含框架层行）、两个 hub 的接口节
...
## [2026-08-03] add | flatten 全流程白话分享文档（给同事）

用户需要一份可分享给同事的白话文档：不要专业术语、交代全部背景、讲清每条支路干什么、解释"为什么 reshape 不注册直接调 kernel"，越详细越直白越好。

**产出**：新建 [[flatten全流程白话分享版]]（wiki/sources/，用户指定位置）——先补三个背景（张量=数据+说明书 / view=换标签 vs copy=搬货 / dispatcher=总机分诊台），再逐层白话讲解 6 层旅程（含医院分诊台类比：flatten 已挂号盖章，reshape 是科室内部直接喊人所以不重新排队；copy_ 因设备而异才必须回总机），附分支大全表、CPU/设备与数据操作总表、快问快答（会不会复制数据/GPU 动不动手/开销大不大）。

**同步**：[[index]] / [[overview]]（33 页）。
