...
- **hub**：知识大类的粘合页（MOC），含大类范围、推荐阅读顺序、成员页清单；每个大类一个（如 PyTorch、vLLM 推理框架）
...
**用户只管提问和策展来源，不会显式要求"新建大类"。** 每次 ingest / query 沉淀时，LLM 必须执行归属判断：

1. **判断**：新内容属于哪个现有大类？（对照 `wiki/index.md` 的大类分区）
2. **若不属于任何现有大类 → 自动创建新大类**：
   - 新建 hub 页 `wiki/hubs/<大类名>.md`（范围、主线、阅读顺序、成员清单）
   - `wiki/index.md` 新增对应分区；`wiki/overview.md` 大类导航同步
   - `wiki/log.md` 记录本次大类创建
3. **新大类内容还少（1~2 页）时**：可先以单页为入口、在 index 中标注，成员页 ≥3 时再补 hub
4. **大类命名规则（红线）**：**大类名 = 领域名，不是当前内容的角度名**
   - ✅ `PyTorch`（领域：机制/实现/算法/API 都装得下）
   - ❌ `PyTorch 源码机制`（角度名：内容扩展到实现/算法时名字就失效）
   - 内容角度用 hub 内部的"**子线**"表达（如 PyTorch hub 的"源码机制子线"）
   - 若领域名与既有页面撞名（如实体页 [[vLLM]]），用大领域短语（如 `vLLM 推理框架`）
5. 同理，**已有大类的内容扩展到新角度时**：不新建大类，在该大类 hub 内新增子线
